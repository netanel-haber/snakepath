...
TEST(posix_anchor_empty) {
    SpPath p = sp_path("", .flavor = SP_FLAVOR_POSIX);
    ASSERT_SV_EQ(sp_anchor(&p), "");
}

TEST(posix_anchor_relative) {
    SpPath p = sp_path("a/b", .flavor = SP_FLAVOR_POSIX);
    ASSERT_SV_EQ(sp_anchor(&p), "");
}
...
TEST(posix_name_with_ext) {
    SpPath p = sp_path("a/b.py", .flavor = SP_FLAVOR_POSIX);
...
TEST(posix_stem_hidden) {
    SpPath p = sp_path("a/.hgrc", .flavor = SP_FLAVOR_POSIX);
    ASSERT_SV_EQ(sp_stem(&p), ".hgrc");
}

TEST(posix_stem_multi_ext) {
    SpPath p = sp_path("a/b.tar.gz", .flavor = SP_FLAVOR_POSIX);
...
TEST(posix_suffix_hidden_with_ext) {
    SpPath p = sp_path("a/.hg.rc", .flavor = SP_FLAVOR_POSIX);
...
TEST(posix_parts_count) {
    SpPath p = sp_path("/a/b/c", .flavor = SP_FLAVOR_POSIX);
...
TEST(posix_parent_single) {
    SpPath p = sp_path("a", .flavor = SP_FLAVOR_POSIX);
    ASSERT_PATH_EQ(sp_parent(&p), ".");
}
...
TEST(posix_is_relative_to_yes) {
    SpPath p = sp_path("a/b", .flavor = SP_FLAVOR_POSIX);
    SpPath o = sp_path("a", .flavor = SP_FLAVOR_POSIX);
    ASSERT(sp_is_relative_to(&p, &o));
}

TEST(posix_is_relative_to_no) {
    SpPath p = sp_path("a/b", .flavor = SP_FLAVOR_POSIX);
    SpPath o = sp_path("c", .flavor = SP_FLAVOR_POSIX);
    ASSERT(!sp_is_relative_to(&p, &o));
}
...
TEST(posix_relative_to_self) {
    SpPath p = sp_path("a/b", .flavor = SP_FLAVOR_POSIX);
    SpPath o = sp_path("a/b", .flavor = SP_FLAVOR_POSIX);
    SpPath r = sp_relative_to(&p, &o);
    ASSERT_PATH_EQ(r, ".");
}
...
TEST(posix_eq) {
    SpPath a = sp_path("a/b", .flavor = SP_FLAVOR_POSIX);
    SpPath b = sp_path("a/b", .flavor = SP_FLAVOR_POSIX);
    ASSERT(sp_eq(a, b));
}
...
TEST(win_drive_letter) {
    SpPath p = sp_path("C:/a/b", .flavor = SP_FLAVOR_WINDOWS);
    ASSERT_SV_EQ(sp_drive(&p), "C:");
}

TEST(win_drive_empty) {
    SpPath p = sp_path("/a/b", .flavor = SP_FLAVOR_WINDOWS);
    ASSERT_SV_EQ(sp_drive(&p), "");
}
...
TEST(win_unc_parts) {
    SpPath p = sp_path("//server/share/a/b", .flavor = SP_FLAVOR_WINDOWS);
    SpPartsIter it = sp_parts_begin(&p);
    SpStr part;
    ASSERT(sp_parts_next(&it, &part));
...
TEST(win_name) {
    SpPath p = sp_path("C:/a/b.py", .flavor = SP_FLAVOR_WINDOWS);
    ASSERT_SV_EQ(sp_name(&p), "b.py");
}
...
TEST(win_suffixes_multi) {
    SpPath p = sp_path("c:a/b.tar.gz", .flavor = SP_FLAVOR_WINDOWS);
    SpSuffixes s = sp_suffixes(&p);
    ASSERT(s.count == 2);
    ASSERT_SV_EQ(s.items[0], ".tar");
    ASSERT_SV_EQ(s.items[1], ".gz");
...
TEST(win_parent) {
    SpPath p = sp_path("C:/a/b/c", .flavor = SP_FLAVOR_WINDOWS);
...
TEST(win_is_absolute_unc) {
    SpPath p = sp_path("//server/share/a", .flavor = SP_FLAVOR_WINDOWS);
    ASSERT(sp_is_absolute(&p));
}
...
TEST(win_relative_to) {
    SpPath p = sp_path("C:/a/b/c", .flavor = SP_FLAVOR_WINDOWS);
    SpPath o = sp_path("C:/a", .flavor = SP_FLAVOR_WINDOWS);
    SpPath r = sp_relative_to(&p, &o);
    ASSERT_PATH_EQ(r, "b\\c");
}
...
TEST(win_is_relative_to_diff_drive) {
    SpPath p = sp_path("C:/a/b", .flavor = SP_FLAVOR_WINDOWS);
    SpPath o = sp_path("D:/a", .flavor = SP_FLAVOR_WINDOWS);
    ASSERT(!sp_is_relative_to(&p, &o));
}
...
TEST(edge_complex_suffixes) {
    SpPath p = sp_path("a/b.c.d.e", .flavor = SP_FLAVOR_POSIX);
    SpSuffixes s = sp_suffixes(&p);
    ASSERT(s.count == 3);
...
TEST(edge_hidden_file_multi_ext) {
    SpPath p = sp_path(".tar.gz", .flavor = SP_FLAVOR_POSIX);
    ASSERT_SV_EQ(sp_stem(&p), ".tar");
    ASSERT_SV_EQ(sp_suffix(&p), ".gz");
}

TEST(edge_single_component) {
    SpPath p = sp_path("file.txt", .flavor = SP_FLAVOR_POSIX);
...
TEST(edge_deeply_nested) {
    SpPath p = sp_path("/a/b/c/d/e/f/g", .flavor = SP_FLAVOR_POSIX);
